 ...
 */
 function plgRDmediaNovalnetCc( &$subject, $params  ) {
	...
	## load plugin params info
 	$plugin =& JPluginHelper::getPlugin('novalnetcc', 'novalnetcc');
	...
	$this->return_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetcc&Itemid='.$this->ordercode;
	$this->cancel_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetcc_failed&Itemid='.$this->ordercode;
	$this->form_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetcc_form&Itemid='.$this->ordercode;
...
*/
	 ...
	 function display()
	 {
		...
		$document->addStyleSheet( JURI::root(true).'/plugins/rdmedia/novalnetcc/rdmedia_novalnetcc/css/novalnetcc.css' );	
		$session =& JFactory::getSession();
		$user =& JFactory::getUser();
		$ordertotal = round($this->amount, 2) * 100;
			...
			if(!isset($this->novalnet_response['tid']) && empty($this->novalnet_response['tid'])){				
				if($this->layout == 1 && $isJ30 == true ){			 
					echo '<img src="'.$this->nnSiteUrl.'www.novalnet.de/img/NN_Logo_T.png" alt="'.JText::_('PLG_NOVALNETCC_NOVALNET_LOGO_ALT').'" title="Novalnet AG" />'.JText::_('PLG_NOVALNETCC_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETCC_FRONTEND_TESTMODE_DESC')."</span>";
					}
					...
					echo    '<button class="btn btn-block btn-success" alt="'.JText::_('PLG_NOVALNETCC_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETCC_PAYMENT_LOGO_TITLE').'" style="margin-top: 8px;" type="submit" id="enter_cc" name="enter_cc">'.JText::_( 'Make Payment' ).'</button>';		
					echo 	'</form>';
				}else{
					echo '<form action="'.$this->form_url.'" method="post" name="novalnetForm">';
					echo '<div id="plg_rdmedia_novalnetcc" alt="'.JText::_('PLG_NOVALNETCC_NOVALNET_LOGO_ALT').'" title="Novalnet AG">';
					echo '<div id="plg_rdmedia_novalnetcc_cards">'.JText::_('PLG_NOVALNETCC_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETCC_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '</div>';
					...
					echo implode('', $inputvalues);					
					echo '<div id="plg_rdmedia_novalnetcc_confirmbutton">';
					echo    '<input type="submit" id="enter_cc" name="enter_cc" value="" class="novalnetcc_button" alt="'.JText::_('PLG_NOVALNETCC_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETCC_PAYMENT_LOGO_TITLE').'" style="width: 116px;">';
					echo '</div>';	
					echo '</div>';
					echo '</form>';
				}
	...
	function doValidation() {
		...
		$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCC_BACK_END_ERR'));
		return false;	
 		}	
 		elseif($this->manual_limit && (!preg_match('/^\d+$/',$this->manual_limit) || !preg_match('/^\d+$/',$this->second_product_id) || !preg_match('/^\d+$/',$this->second_tariff_id))){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCC_MANUAL_LIMIT_ERR'));
			return false;
		}
		if(!$this->email || !$this->firstname || !$this->lastname){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCC_CUST_NAME_EMAIL_ERR'));
			return false;
		}
		return true;	   
	}
	...
	function _postBackParam(){
		$callBackParams = array(
            'vendor' => $this->novalnet_request['vendor_id'],
		    'auth_code' => $this->novalnet_request['vendor_authcode'],
            'product' => $this->novalnet_request['product_id'],
            'tariff' => $this->novalnet_request['tariff_id'],
	...
	function novalnetcc_form(){
		...
		$response = $session->set("novalnet_request_cc", $this->novalnet_request);		 
		echo '<div class="component-content">
		      <div class="item-page">';
		echo '<form action="'.$this->payport_url.'" method="post" id="novalnetCc" name="novalnetCc" target="my_iframe">
		      <fieldset >';
			foreach($this->novalnet_request as $k => $v) {		
		 		$inputvalues[] = '<input type="hidden" name="' . $k . '" value="' . $v . '" />' . "\n";		
			}
			echo implode('', $inputvalues);
			echo '<iframe id="my_iframe" name="my_iframe" width="550" height="410" src="'.$this->nnSiteUrl.'www.novalnet.de/img/novalnet-loading-icon.gif" frameborder="0"></iframe>
			</fieldset>
			</form>
		  	</div>
		     </div>
		     <script>document.getElementById("novalnetCc").submit();</script>';
	}	
	...
	function Novalnet_prepareComment() {	
		...
		if($this->novalnet_response['status'] == '100') {	
			if($this->Novalnet_TestOrder() == 1) {
				$note .= "<b>" . JText::_('PLG_NOVALNETCC_TEST_ORDER') . "</b>" . $sNewLine;
			}
				$note .= "<b>" . JText::_('PLG_NOVALNETCC_PAYMENT_NAME') . "</b>" . $sNewLine;	
				$note .= JText::_('PLG_NOVALNETCC_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine . $sNewLine;
		}else{
			...
			$note .= "<b>" . JText::_('PLG_NOVALNETCC_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETCC_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine;
	...
	function novalnetcc() {
		$session =& JFactory::getSession();
		$this->novalnet_response = $_POST;
		$this->novalnet_request = $session->get('novalnet_request_cc');			
		$db = JFactory::getDBO();
		...
		$checkhash = $this->generateHash(array('auth_code' => $this->novalnet_response['vendor_authcode'], 'product_id' => $this->novalnet_response['product_id'], 'tariff' => $this->novalnet_response['tariff_id'], 'amount' => $this->novalnet_response['amount'], 'test_mode' => $this->novalnet_response['test_mode'], 'uniqid' => $this->novalnet_response['uniqid']));
		if($this->novalnet_response['hash2'] != $checkhash){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCC_CHECKHASH_ERR'));
			return false;
		...
		$this->novalnet_request['product_id'] = $this->doDecode($this->novalnet_request['product_id']);
		$this->novalnet_request['tariff_id'] = $this->doDecode($this->novalnet_request['tariff_id']);
		$this->novalnet_request['vendor_authcode'] = $this->doDecode($this->novalnet_request['vendor_authcode']);
