 ...
 */
 function plgRDmediaNovalnetInvoice( &$subject, $params  ) {
	...
	## load plugin params info
 	$plugin =& JPluginHelper::getPlugin('novalnetinvoice', 'novalnetinvoice');
	...
	$this->return_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetinvoice&Itemid='.$this->ordercode;
	$this->cancel_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetinvoice_failed&Itemid='.$this->ordercode;	
 }
...
*/
	 ...
	 function display()
	 {
		...
		$ordertotal = round($this->amount, 2) * 100;//number_format($this->amount, 2, '.', '');		
			...
			if($this->doValidation()){
				...
				if(preg_match('/^\d+$/',$this->payment_period)){
					$this->novalnet_request['due_date'] = $this->_getDueDate($this->payment_period);
				}
				$this->novalnet_request = array_map("trim", $this->novalnet_request);
				$request = $session->set("novalnet_request_invoice", $this->novalnet_request);
			if(!isset($this->novalnet_response['tid']) && empty($this->novalnet_response['tid'])){				
				if($this->layout == 1 && $isJ30 == true ){			 
					echo '<img src="'.$this->nnSiteUrl.'www.novalnet.de/img/NN_Logo_T.png" alt="'.JText::_('PLG_NOVALNETINVOICE_NOVALNET_LOGO_ALT').'" title="Novalnet AG" />'.JText::_('PLG_NOVALNETINVOICE_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETINVOICE_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '<form action="" method="post" name="novalnetForm">';
					echo    '<button class="btn btn-block btn-success" alt="'.JText::_('PLG_NOVALNETINVOICE_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETINVOICE_PAYMENT_LOGO_TITLE').'" style="margin-top: 8px;" type="submit" name="enter_invoice">'.JText::_( 'Make Payment' ).'</button>';			
					echo 	'</form>';	
				}else{
					echo '<form action="" method="post" name="novalnetForm">';
					echo '<div id="plg_rdmedia_novalnetinvoice" alt="'.JText::_('PLG_NOVALNETINVOICE_NOVALNET_LOGO_ALT').'" title="Novalnet AG">';
					echo '<div id="plg_rdmedia_novalnetinvoice_cards">'.JText::_('PLG_NOVALNETINVOICE_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETINVOICE_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '</div>';
					echo '<div id="plg_rdmedia_novalnetinvoice_confirmbutton">';
					echo    '<input type="submit" name="enter_invoice" value="" class="novalnetinvoice_button" alt="'.JText::_('PLG_NOVALNETINVOICE_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETINVOICE_PAYMENT_LOGO_TITLE').'" style="width: 116px;">';
					echo '</div>';	
					echo '</div>';
					echo '</form>';
				}
			}
			if(isset($_REQUEST['enter_invoice']) && empty($this->novalnet_response['tid'])){
				...
				$response = $session->set("novalnet_response_invoice", $this->novalnet_response);
	...
	function novalnetinvoice_failed(){
		$db = JFactory::getDBO();
		$session =& JFactory::getSession();
		$this->novalnet_response = $session->get('novalnet_response_invoice');
		...
		$session->clear('novalnet_response_invoice');
	...
	function _getDueDate($days=0) {
		return $days == '' ? NULL : date('d.m.Y', strtotime('+' . max(0, intval($days)) . ' days'));
	}
	...
	function doValidation() {
		 if(!$this->is_digits($this->vendor_id) || !$this->is_digits($this->product_id) || !$this->auth_code || !$this->is_digits($this->tariff_id)){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETINVOICE_BACK_END_ERR'));
			return false;	
 		}
		if(!$this->email || !$this->firstname || !$this->lastname){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETINVOICE_CUST_NAME_EMAIL_ERR'));
			return false;
		}
		return true;	   
	}
	...
	function Novalnet_prepareComment() {	
		...
		if($this->novalnet_response['status'] == '100'){	
			if($this->Novalnet_TestOrder() == 1) {
				$note .= "<b>" . JText::_('PLG_NOVALNETINVOICE_TEST_ORDER') . "</b>" . $sNewLine;
			}
			$note .= "<b>" . JText::_('PLG_NOVALNETINVOICE_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETINVOICE_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine . $sNewLine;
			$note .= "<b>" . JText::_('PLG_NOVALNETINVOICE_TRANSFER_NOTE') . "</b>" . $sNewLine; 
		if(isset($this->novalnet_request['due_date']) && $this->novalnet_request['due_date']!='') {
			$note .= JText::_('PLG_NOVALNETINVOICE_DUE_DATE') . " <b>" . $this->novalnet_request['due_date'] . "</b>";
			$note .= $sNewLine;
		}
			$sAmt = str_replace(".", ",", $this->novalnet_response['amount']);
			$note .= JText::_('PLG_NOVALNETINVOICE_ACCOUNT_HOLDER') . " <b>NOVALNET AG</b>" . $sNewLine;
			$note .= JText::_('PLG_NOVALNETINVOICE_ACCOUNT_NUMBER') . " <b>" . $this->novalnet_response['invoice_account'] . "</b>" . $sNewLine;
			$note .= JText::_('PLG_NOVALNETINVOICE_BANK_CODE') . " <b>" . $this->novalnet_response['invoice_bankcode'] . "</b>" . $sNewLine;
			$note .= JText::_('PLG_NOVALNETINVOICE_BANK') . " <b>" . $this->novalnet_response['invoice_bankname'] .' '.$this->novalnet_response['invoice_bankplace']."</b>";
			$note .= JText::_('PLG_NOVALNETINVOICE_AMOUNT') . " <b>" . $sAmt . ' ' . "&euro;" . "</b>" . $sNewLine;
			$note .= JText::_('PLG_NOVALNETINVOICE_REFERENCE') . " TID - <b>" . $this->novalnet_response['tid'] . "</b>". $sNewLine . $sNewLine;
			$note .= "<b>".JText::_('PLG_NOVALNETINVOICE_INTERNATIONAL_TRANSFER'). "</b>". $sNewLine;
			$note .= JText::_('PLG_NOVALNETINVOICE_IBAN') . " <b>" . $this->novalnet_response['invoice_iban'] .  "</b>". $sNewLine;
			$note .= JText::_('PLG_NOVALNETINVOICE_SWIFT_BIC') . " <b>" . $this->novalnet_response['invoice_bic'] .  "</b>". $sNewLine;
		}else{
			...
			$note .= "<b>" . JText::_('PLG_NOVALNETINVOICE_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETINVOICE_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine;
	...
	function novalnetinvoice() {
		...
		## Gettig the orderid if there is one.
		$this->novalnet_response = $session->get('novalnet_response_invoice');
		$this->novalnet_request = $session->get('novalnet_request_invoice');			
		...
		$query = 'UPDATE #__ticketmaster_orders SET paid = 3, published = 1 WHERE ordercode = '.(int)$this->ordercode.'';
		$db->setQuery($query);
