...
class plgRDmediaNovalnetAustria extends JPlugin
{
/**
 * Constructor
 *
 ...
 */
 function plgRDmediaNovalnetAustria( &$subject, $params  ) {
 
    parent::__construct( $subject , $params  );
	
	## Loading language:	
	$lang = JFactory::getLanguage();
	$lang->load('plg_rdmedia_novalnetaustria', JPATH_ADMINISTRATOR);	

	## load plugin params info
 	$plugin =& JPluginHelper::getPlugin('novalnetaustria', 'novalnetaustria');
	...
	$this->payment_type = 'novalnet_austria';
	...
	$this->return_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetaustria&Itemid='.$this->ordercode;
	$this->cancel_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetaustria_failed&Itemid='.$this->ordercode;
	$this->form_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetaustria_form&Itemid='.$this->ordercode;
 }
...
*/
	 ...
	 function display()
	 {
		...
		$document->addStyleSheet( JURI::root(true).'/plugins/rdmedia/novalnetaustria/rdmedia_novalnetaustria/css/novalnetaustria.css' );	
		$session =& JFactory::getSession();
		$user =& JFactory::getUser();
		...
		## Check the amount, if higher then 0.00 then show the plugin data.	
			...
			if(!isset($this->novalnet_response['tid']) && empty($this->novalnet_response['tid'])){				
				if($this->layout == 1 && $isJ30 == true ){			 
					echo '<img src="'.$this->nnSiteUrl.'www.novalnet.de/img/NN_Logo_T.png" alt="'.JText::_('PLG_NOVALNETAUSTRIA_NOVALNET_LOGO_ALT').'" title="Novalnet AG" />'.JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETAUSTRIA_FRONTEND_TESTMODE_DESC')."</span>";
					}
					...
					echo    '<button class="btn btn-block btn-success" alt="'.JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_LOGO_TITLE').'" style="margin-top: 8px;" type="submit" id="enter_austria" name="enter_austria">'.JText::_( 'Make Payment' ).'</button>';		
					echo 	'</form>';
				}else{
					echo '<form action="'.$this->form_url.'" method="post" name="novalnetForm">';				
					echo '<div id="plg_rdmedia_novalnetaustria" alt="'.JText::_('PLG_NOVALNETAUSTRIA_NOVALNET_LOGO_ALT').'" title="Novalnet AG">';				
					echo '<div id="plg_rdmedia_novalnetaustria_cards">'.JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETAUSTRIA_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '</div>';
					...
					echo '<div id="plg_rdmedia_novalnetaustria_confirmbutton">';
					echo    '<input type="submit" id="enter_austria" name="enter_austria" value="" class="novalnetaustria_button" alt="'.JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_LOGO_TITLE').'" style="width: 116px;">';
					echo '</div>';				
					echo '</div>';				
					echo '</form>';
				}
	...
	function novalnetaustria_failed(){
		$db = JFactory::getDBO();
		$session =& JFactory::getSession();
		$this->novalnet_response = $session->get('novalnet_response_austria');
		...
		$session->clear('novalnet_request_austria');
		$session->clear('novalnet_response_austria');	
	...
	function doValidation() {
		if(!$this->is_digits($this->vendor_id) || !$this->is_digits($this->product_id) || !$this->auth_code || !$this->is_digits($this->tariff_id)){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETAUSTRIA_BACK_END_ERR'));
			return false;	
 		}	
 		elseif($this->manual_limit && (!preg_match('/^\d+$/',$this->manual_limit) || !preg_match('/^\d+$/',$this->second_product_id) || !preg_match('/^\d+$/',$this->second_tariff_id))){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETAUSTRIA_MANUAL_LIMIT_ERR'));
			return false;
		}
		if(!$this->email || !$this->firstname || !$this->lastname){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETAUSTRIA_CUST_NAME_EMAIL_ERR'));
			return false;
		}
		return true;	   
	}
	...
	function novalnetaustria_form(){
		$session =& JFactory::getSession();
		if(isset($_REQUEST['enter_austria'])){
			$session->set("novalnet_request_austria", $_POST);
		} 
		echo '<div class=component-content">
		      <div class=item-page">
		      <form action="" method="post" name="novalnetPaymentForm">
  		      <fieldset>
	  		<legend>'.JText::_('PLG_NOVALNETAUSTRIA_REQUIRED_FIELD_LABLE').' :</legend>
    			<div style="vertical-align:top">
			<div style="display:inline-block;vertical-align:top;padding:0px 10px 10px;width:250px;">				
				<label for="kontoinhaber">'.JText::_('PLG_NOVALNETAUSTRIA_ACC_HOLDER').'*:</label>
				  <input type="text" name="kontoinhaber" id="kontoinhaber" value="" autocomplete=OFF /><br />
				<label for="kontonr">'.JText::_('PLG_NOVALNETAUSTRIA_ACC_NUMBER').'*:</label>
				  <input type="text" name="kontonr" id="kontonr" value="" autocomplete=OFF /><br />
				<label for="blz">'.JText::_('PLG_NOVALNETAUSTRIA_BANK_CODE').'*:</label>
				  <input type="text" name="blz" id="blz" value="" autocomplete=OFF /><br />
			</div>
  			</div>
		     </fieldset>
  		     <input type="submit" name="elv_at_form" id="elv_at_form" value="'.JText::_('PLG_NOVALNETAUSTRIA_BUTTON_PAYMENT_PROCEED').'"/>
		     </form>
		     </div>
		     </div>';
		if(isset($_REQUEST['elv_at_form'])){
			...
			$this->novalnet_request = array_merge($session->get('novalnet_request_austria'), $payment_params);
			$this->novalnet_request = array_map("trim", $this->novalnet_request);
			...
			$response = $session->set("novalnet_response_austria", $this->novalnet_response);
	...
	function doFormValidation(){
		if(!preg_match("/^\d{5,}$/",$this->novalnet_request['kontonr']) || !preg_match("/^\d{3,}$/",$this->novalnet_request['blz']) || preg_match("/[#%\^<>@$=*!]+/i",$this->novalnet_request['kontoinhaber'])){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_ERR'));
			return false;
		}
		return true;
	}			
	...
	function Novalnet_prepareComment() {	
		...
		if($this->novalnet_response['status'] == '100'){
			if($this->Novalnet_TestOrder() == 1) {
				$note .= "<b>" . JText::_('PLG_NOVALNETAUSTRIA_TEST_ORDER') . "</b>" . $sNewLine;
			}
				$note .= "<b>" . JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_NAME') . "</b>" . $sNewLine;	
				$note .= JText::_('PLG_NOVALNETAUSTRIA_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine . $sNewLine;
		}else{
			...
			$note .= "<b>" . JText::_('PLG_NOVALNETAUSTRIA_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETAUSTRIA_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine;
			$note .= $status . $sNewLine . $sNewLine;
		}
		return $note;
	}

	function novalnetaustria() {		
		## Getting the global DB session
		$session =& JFactory::getSession();
		## Gettig the orderid if there is one.
		$this->novalnet_response = $session->get('novalnet_response_austria');	
		$this->novalnet_request = $session->get('novalnet_request_austria');			
