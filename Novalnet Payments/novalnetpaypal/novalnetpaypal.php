 ...
 */
 function plgRDmediaNovalnetPaypal( &$subject, $params  ) {
	...
	## load plugin params info
 	$plugin =& JPluginHelper::getPlugin('novalnetpaypal', 'novalnetpaypal');
	...
	$this->api_user = trim($this->params->def('api_user'));
	$this->api_password = trim($this->params->def('api_pswd'));
	$this->api_signature = trim($this->params->def('api_signature'));
	...
	$this->return_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetpaypal&Itemid='.$this->ordercode;
	$this->cancel_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetpaypal_failed&Itemid='.$this->ordercode;
...
*/
	 ...
	 function display()
	 {
		...
		$ordertotal = round($this->amount, 2) * 100;//number_format($this->amount, 2, '.', '');		
			...
			if($this->doValidation()){			
				...
				$api_user = $this->doEncode($this->api_user);
				$api_pswd = $this->doEncode($this->api_password);
				$api_sign = $this->doEncode($this->api_signature);	
			...
			if(!isset($this->novalnet_response['tid']) && empty($this->novalnet_response['tid'])){				
				if($this->layout == 1 && $isJ30 == true ){			 
					echo '<img src="'.$this->nnSiteUrl.'www.novalnet.de/img/NN_Logo_T.png" alt="'.JText::_('PLG_NOVALNETPAYPAL_NOVALNET_LOGO_ALT').'" title="Novalnet AG" />'.JText::_('PLG_novalnetpaypal_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETPAYPAL_FRONTEND_TESTMODE_DESC')."</span>";
					}
					...
					echo    '<button class="btn btn-block btn-success" alt="'.JText::_('PLG_NOVALNETPAYPAL_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETPAYPAL_PAYMENT_LOGO_TITLE').'" style="margin-top: 8px;" type="submit" id="enter_paypal" name="enter_paypal">'.JText::_( 'Make Payment' ).'</button>';		
					echo 	'</form>';
				}else{
					echo '<form action="'.$this->payport_url.'" method="post" name="novalnetForm">';
					echo '<div id="plg_rdmedia_novalnetpaypal" alt="'.JText::_('PLG_NOVALNETPAYPAL_NOVALNET_LOGO_ALT').'" title="Novalnet AG">';
					echo '<div id="plg_rdmedia_novalnetpaypal_cards">'.JText::_('PLG_NOVALNETPAYPAL_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETPAYPAL_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '</div>';
					...
					echo implode('', $inputvalues);				
					echo '<div id="plg_rdmedia_novalnetpaypal_confirmbutton">';
					echo    '<input type="submit" id="enter_paypal" name="enter_paypal" value="" class="novalnetpaypal_button" alt="'.JText::_('PLG_NOVALNETPAYPAL_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETPAYPAL_PAYMENT_LOGO_TITLE').'" style="width: 116px;">';
					echo '</div>';	
					echo '</div>';
					echo '</form>';
				}
	...
	function doValidation() {
		 if(!$this->is_digits($this->vendor_id) || !$this->is_digits($this->product_id) || !$this->auth_code || !$this->is_digits($this->tariff_id) || !$this->payment_key || !$this->api_user || !$this->api_password || !$this->api_signature){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETPAYPAL_BACK_END_ERR'));
			return false;
 		}
		if(!$this->email || !$this->firstname || !$this->lastname){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETPAYPAL_CUST_NAME_EMAIL_ERR'));
			return false;	
		}
		return true;	   
	}
	...
	function Novalnet_prepareComment() {	
		$sNewLine = "\n";
		$note = '';
		if(in_array($this->novalnet_response['status'], array('100', '90'))){
			if($this->Novalnet_TestOrder() == 1) {
				$note .= "<b>" . JText::_('PLG_NOVALNETPAYPAL_TEST_ORDER') . "</b>" . $sNewLine;
			}
			$note .= "<b>" . JText::_('PLG_NOVALNETPAYPAL_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETPAYPAL_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine . $sNewLine;
		}else{
			...
			$note .= "<b>" . JText::_('PLG_NOVALNETPAYPAL_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETPAYPAL_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine;
	...
	function novalnetpaypal() {		
		...
		}elseif($this->novalnet_response['status'] == 100 || $this->novalnet_response['status'] == 90){
		...
		if($this->novalnet_response['status'] == 90){
