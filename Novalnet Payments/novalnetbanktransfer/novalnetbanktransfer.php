 ...
 */
 function plgRDmediaNovalnetBanktransfer( &$subject, $params  ) {
	...
	## load plugin params info
 	$plugin =& JPluginHelper::getPlugin('novalnetbanktransfer', 'novalnetbanktransfer');
	...
	$this->return_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetbanktransfer&Itemid='.$this->ordercode;
	$this->cancel_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetbanktransfer_failed&Itemid='.$this->ordercode;
...
*/
	 ...
	 function display()
	 {
		...
		$document->addStyleSheet( JURI::root(true).'/plugins/rdmedia/novalnetbanktransfer/rdmedia_novalnetbanktransfer/css/novalnetbanktransfer.css' );	
		$session =& JFactory::getSession();
		$user =& JFactory::getUser();
		$ordertotal = round($this->amount, 2) * 100;	
			...
			if(!isset($this->novalnet_response['tid']) && empty($this->novalnet_response['tid'])){				
				if($this->layout == 1 && $isJ30 == true ){			 
					echo '<img src="'.$this->nnSiteUrl.'www.novalnet.de/img/NN_Logo_T.png" alt="'.JText::_('PLG_NOVALNETBANKTRANSFER_NOVALNET_LOGO_ALT').'" title="Novalnet AG" />'.JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETBANKTRANSFER_FRONTEND_TESTMODE_DESC')."</span>";
					}
					...
					echo    '<button class="btn btn-block btn-success" alt="'.JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_LOGO_TITLE').'" style="margin-top: 8px;" type="submit" id="enter_bank" name="enter_bank">'.JText::_( 'Make Payment' ).'</button>';		
					echo 	'</form>';
				}else{
					echo '<form action="'.$this->payport_url.'" method="post" name="novalnetForm">';
					echo '<div id="plg_rdmedia_novalnetbanktransfer" alt="'.JText::_('PLG_NOVALNETBANKTRANSFER_NOVALNET_LOGO_ALT').'" title="Novalnet AG">';
					echo '<div id="plg_rdmedia_novalnetbanktransfer_cards">'.JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETBANKTRANSFER_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '</div>';
					...
					echo implode('', $inputvalues);				
					echo '<div id="plg_rdmedia_novalnetbanktransfer_confirmbutton">';
					echo    '<input type="submit" id="enter_bank" name="enter_bank" value="" class="novalnetbanktransfer_button" alt="'.JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_LOGO_TITLE').'" style="width: 116px;">';
					echo '</div>';	
					echo '</div>';
					echo '</form>';
				}
	...
	function doValidation() {
		...
		$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETBANKTRANSFER_BACK_END_ERR'));
		return false;	
 		}
		if(!$this->email || !$this->firstname || !$this->lastname){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETBANKTRANSFER_CUST_NAME_EMAIL_ERR'));
			return false;
		}
		return true;	   
	}
	...
	function Novalnet_prepareComment() {	
		...
		if($this->novalnet_response['status'] == '100'){
			if($this->Novalnet_TestOrder() == 1) {
				$note .= "<b>" . JText::_('PLG_NOVALNETBANKTRANSFER_TEST_ORDER') . "</b>" . $sNewLine;
			}
				$note .= "<b>" . JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_NAME') . "</b>" . $sNewLine;	
				$note .= JText::_('PLG_NOVALNETBANKTRANSFER_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine . $sNewLine;
		}else{
			...
			$note .= "<b>" . JText::_('PLG_NOVALNETBANKTRANSFER_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETBANKTRANSFER_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine;
	...
	function novalnetbanktransfer() {		
		...
		if($this->novalnet_response['hash2'] != $checkhash){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETBANKTRANSFER_CHECKHASH_ERR'));
			return false;
		...
		//$search_query = 'SELECT * FROM #__ticketmaster_transactions WHERE orderid = '.$this->ordercode.'';
