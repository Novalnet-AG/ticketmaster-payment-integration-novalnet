...
class plgRDmediaNovalnetCcsecure extends JPlugin
{
/**
 * Constructor
 *
 ...
 */
 function plgRDmediaNovalnetCcsecure( &$subject, $params  ) {
 
    parent::__construct( $subject , $params  );
	
	## Loading language:	
	$lang = JFactory::getLanguage();
	$lang->load('plg_rdmedia_novalnetccsecure', JPATH_ADMINISTRATOR);	

	## load plugin params info
 	$plugin =& JPluginHelper::getPlugin('novalnetccsecure', 'novalnetccsecure');
	...
	$this->payment_type = 'novalnet_ccsecure';
	$this->paygate_url='https://payport.novalnet.de/paygate.jsp';
	$this->payport_url='https://payport.novalnet.de/global_pci_payport';
	...
	$this->return_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetccsecure&Itemid='.$this->ordercode;
	$this->cancel_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetccsecure_failed&Itemid='.$this->ordercode;

	$this->form_url = JURI::root().'index.php?option=com_ticketmaster&view=transaction&payment_type=novalnetccsecure_form&Itemid='.$this->ordercode;
 }
...
*/
	 ...
	 function display()
	 {
		...
		$document->addStyleSheet( JURI::root(true).'/plugins/rdmedia/novalnetccsecure/rdmedia_novalnetccsecure/css/novalnetccsecure.css' );	
		$session =& JFactory::getSession();
		$user =& JFactory::getUser();
		$ordertotal = round($this->amount, 2) * 100;	
			...
			if(!isset($this->novalnet_response['tid']) && empty($this->novalnet_response['tid'])){				
				if($this->layout == 1 && $isJ30 == true ){			 
					echo '<img src="'.$this->nnSiteUrl.'www.novalnet.de/img/NN_Logo_T.png" alt="'.JText::_('PLG_NOVALNETCCSECURE_NOVALNET_LOGO_ALT').'" title="Novalnet AG" />'.JText::_('PLG_NOVALNETCCSECURE_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETCCSECURE_FRONTEND_TESTMODE_DESC')."</span>";
					}
					...
					echo    '<button class="btn btn-block btn-success" alt="'.JText::_('PLG_NOVALNETCCSECURE_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETCCSECURE_PAYMENT_LOGO_TITLE').'" style="margin-top: 8px;" type="submit" id="enter_ccsecure" name="enter_ccsecure">'.JText::_( 'Make Payment' ).'</button>';			
					echo 	'</form>';
				}else{
					echo '<form action="'.$this->form_url.'" method="post" name="novalnetForm">';
					echo '<div id="plg_rdmedia_novalnetccsecure" alt="'.JText::_('PLG_NOVALNETCCSECURE_NOVALNET_LOGO_ALT').'" title="Novalnet AG">';
					echo '<div id="plg_rdmedia_novalnetccsecure_cards">'.JText::_('PLG_NOVALNETCCSECURE_PAYMENT_NAME_WT_NOVALNET');
					if($this->test_mode == '1'){
						echo "<br /><span style='color:red;'>".JText::_('PLG_NOVALNETCCSECURE_FRONTEND_TESTMODE_DESC')."</span>";
					}
					echo '</div>';
					...
					echo implode('', $inputvalues);				
					echo '<div id="plg_rdmedia_novalnetccsecure_confirmbutton">';
					echo    '<input type="submit" id="enter_ccsecure" name="enter_ccsecure" value="" class="novalnetccsecure_button" alt="'.JText::_('PLG_NOVALNETCCSECURE_PAYMENT_LOGO_ALT').'" title="'.JText::_('PLG_NOVALNETCCSECURE_PAYMENT_LOGO_TITLE').'" style="width: 116px;">';
					echo '</div>';	
					echo '</div>';
					echo '</form>';
				}
	...
	function novalnetccsecure_failed(){
		$db = JFactory::getDBO();
		...
		$session->clear('novalnet_request_ccsecure');
		$session->clear('novalnet_response');	
	...
	function doValidation() {
		...
		$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCCSECURE_BACK_END_ERR'));
		return false;	
 		}	
 		elseif($this->manual_limit && (!preg_match('/^\d+$/',$this->manual_limit) || !preg_match('/^\d+$/',$this->second_product_id) || !preg_match('/^\d+$/',$this->second_tariff_id))){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCCSECURE_MANUAL_LIMIT_ERR'));
			//header( "Location: ". $this->cancel_url."&message=".$error);
			//$this->_showmsg($this->failure_tpl, $error);
			return false;
		}
		if(!$this->email || !$this->firstname || !$this->lastname){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCCSECURE_CUST_NAME_EMAIL_ERR'));
			return false;
		}
		return true;	   
	}
	...
	function novalnetccsecure_form(){
		$session =& JFactory::getSession();
		if(isset($_REQUEST['enter_ccsecure'])){
			$session->set("novalnet_request_ccsecure", $_POST);
		} 
		echo '<div class=component-content">
		      <div class=item-page">
		      <form action="" method="post" name="novalnetPaymentForm">
  		      <fieldset>
	  		<legend>'.JText::_('PLG_NOVALNETCCSECURE_REQUIRED_FIELD_LABLE').' :</legend>
    			<div style="vertical-align:top">
			<div style="display:inline-block;vertical-align:top;padding:0px 10px 10px;width:250px;">				
				<label for="kontoinhaber">'.JText::_('PLG_NOVALNETCCSECURE_ACC_HOLDER').'*:</label>
				  <input type="text" name="kontoinhaber" id="kontoinhaber" value="" autocomplete=OFF /><br />
				<label for="kontonr">'.JText::_('PLG_NOVALNETCCSECURE_ACC_NUMBER').'*:</label>
				  <input type="text" name="kontonr" id="kontonr" value="" autocomplete=OFF /><br />
				<label for="card_month">'.JText::_('PLG_NOVALNETCCSECURE_EXPIRED_DATE').'*:</label>  
				<select name="card_month" id="card_month" style="width:80px;">
				  <option value="0">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH').'</option>
				  <option value="1">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_JAN').'</option>
				  <option value="2">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_FEB').'</option>
				  <option value="3">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_MAR').'</option>
				  <option value="4">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_APR').'</option>
				  <option value="5">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_MAY').'</option>
				  <option value="6">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_JUN').'</option>
				  <option value="7">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_JUL').'</option>
				  <option value="8">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_AUG').'</option>
				  <option value="9">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_SEP').'</option>
				  <option value="10">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_OCT').'</option>
				  <option value="11">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_NOV').'</option>
				  <option value="12">'.JText::_('PLG_NOVALNETCCSECURE_CARD_MONTH_DEC').'</option>
			  </select>
			  <select name="card_year" id="card_year" style="width:70px;">
				<option value="0">'.JText::_('PLG_NOVALNETCCSECURE_CARD_YEAR').'</option>';
				$today = getdate();		  
				for ($i = $today['year']; $i < $today['year'] + 10; $i++) { 
				$expires_year[] = strftime('%Y', mktime(0, 0, 0, 1, 1, $i)); 
				}
				foreach($expires_year as $k => $v){
				echo '<option value='. $v .'>'. $v .'</option>';	
				 }
			  echo '</select>	
				<label for="cvc">'.JText::_('PLG_NOVALNETCCSECURE_CVC').'*:</label>
				  <input type="text" name="cvc" id="cvc" maxlength = "4" value="" autocomplete=OFF /><br />
				<label for="cvc_details">*'.JText::_('PLG_NOVALNETCCSECURE_CVC_DETAIL').'</label>  
			 </div>
  			 </div>
		     </fieldset>
  		     <input type="submit" name="ccsecure_form" id="ccsecure_form" value="'.JText::_('PLG_NOVALNETCCSECURE_BUTTON_PAYMENT_PROCEED').'"/>
		     </form>
		     </div>
		     </div>';
		if(isset($_REQUEST['ccsecure_form'])){
			$_POST['kontonr'] = str_replace(" ", "", $_POST['kontonr']);
			$this->novalnet_request = $_POST;		
			$this->novalnet_request = array_map("trim", $this->novalnet_request);
			if($this->doFormValidation()){
			$payment_params = array(
					'cc_holder' => $this->novalnet_request['kontoinhaber'],
					'cc_no' => $this->novalnet_request['kontonr'],
					'cc_exp_month' => $this->novalnet_request['card_month'],
					'cc_exp_year' => $this->novalnet_request['card_year'],
					'cc_cvc2' => $this->novalnet_request['cvc'],
					);      			
		
			$this->novalnet_request = array_merge($session->get('novalnet_request_ccsecure'), $payment_params);
			$this->novalnet_request = array_map("trim", $this->novalnet_request);
			echo '<label for="redirection_msg">'.JText::_('PLG_NOVALNETCCSECURE_REDIRECT_MESSAGE').'</label>';
			echo '<form action="'.$this->payport_url.'" method="post" name="novalnetCc3dForm">';
				foreach($this->novalnet_request as $k => $v) {		
					$inputvalues[] = '<input type="hidden" name="' . $k . '" value="' . $v . '" />' . "\n";		
				}
				echo implode('', $inputvalues);				
				echo    '<input type="submit" id="cc3d_form_payment" name="cc3d_form_payment" value="'.JText::_('PLG_NOVALNETCCSECURE_REDIRECT_VALUE').'" onClick="document.forms.novalnetCc3dForm.submit();document.getElementById(\'cc3d_form_payment\').disabled=\'true\';"/></form>';
				echo '<script>document.forms.novalnetCc3dForm.submit();</script>';
			}
		}
	}

	function doFormValidation(){
		if(!preg_match("/^\d{12,}$/",$this->novalnet_request['kontonr']) || !preg_match("/^\d{3,}$/",$this->novalnet_request['cvc']) || preg_match("/[#%\^<>@$=*!]+/i",$this->novalnet_request['kontoinhaber']) || empty($this->novalnet_request['card_month']) || empty($this->novalnet_request['card_year']) || ($this->novalnet_request['card_year'] == date('Y') && $this->novalnet_request['card_month'] < date('m'))){
			$error =  JError::raiseWarning(500, JText::_('PLG_NOVALNETCCSECURE_PAYMENT_ERR'));
			return false;
		}
		return true;
	}	
	...
	function Novalnet_prepareComment() {	
		...
		if($this->novalnet_response['status'] == '100') {
			if($this->Novalnet_TestOrder() == 1) {
				$note .= "<b>" . JText::_('PLG_NOVALNETCCSECURE_TEST_ORDER') . "</b>" . $sNewLine;
			}
				$note .= "<b>" . JText::_('PLG_NOVALNETCCSECURE_PAYMENT_NAME') . "</b>" . $sNewLine;	
				$note .= JText::_('PLG_NOVALNETCCSECURE_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine . $sNewLine;
		}else{
			...
			$note .= "<b>" . JText::_('PLG_NOVALNETCCSECURE_PAYMENT_NAME') . "</b>" . $sNewLine;	
			$note .= JText::_('PLG_NOVALNETCCSECURE_TRANSACTION_ID_MESSAGE') . ' ' . "<b>" . $this->novalnet_response['tid'] ."</b>" . $sNewLine;
			$note .= $status . $sNewLine . $sNewLine;
		}
		return $note;
	}

	function novalnetccsecure() {
		$session =& JFactory::getSession();
		$this->novalnet_request = $session->get('novalnet_request_ccsecure');	
		$this->novalnet_response = $_POST;			
		$db = JFactory::getDBO();
